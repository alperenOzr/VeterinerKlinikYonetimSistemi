...
public partial class MessageBoxForm : Form
{
    ...
    private void SecBtnOlustur()
    {
        Button btn = new()
        {
            Text = "Seç", // Butonun metni
            Location = new Point(100, topPosition), // Butonun konumu
            Width = comboBoxList[0].Width,
            Height = comboBoxList[0].Height
        };
        btn.Click += Btn_Click;
    ...
    private void Btn_Click(object sender, EventArgs e)
    {
        ...
        int id = Convert.ToInt32(comboBoxList[0].SelectedValue);
        HayvanlariYukle(id);
        //MessageBox.Show($"{id}");
    }
    ...
    private void HayvanlariYukle(int id)
    {
        Label label = new()
        {
            Text = "Hayvan",
        ...
        string connectionString = "Server=ALPEREN\\SQLEXPRESS;Database=KlinikYonetimSistemi;User ID=sa;Password=Alperen1; Encrypt=False;";
        string query = "SELECT HayvanID, Isim FROM Hayvanlar WHERE SahipID = @id";

        using SqlConnection connection = new(connectionString);
        using SqlCommand command = new(query, connection);
        command.Parameters.AddWithValue("@id", id);  // id parametresini sorguya ekliyoruz
        try
        {
            connection.Open();
            SqlDataReader reader = command.ExecuteReader();
            ...
            comboBox.DataSource = dataTable;
            comboBox.ValueMember = "HayvanID"; // Arka planda HayvanID tutulacak
            comboBox.DisplayMember = "Isim";   // Görünen değer Hayvan İsmi
        }
        catch (Exception ex)
        {
            MessageBox.Show("Veritabanı hatası: " + ex.Message);
    ...
    private void MuayeneEkle()
    {
        // TextBox'lardaki verilerin kontrolü
        if (string.IsNullOrEmpty(textBoxList[0].Text) || string.IsNullOrEmpty(textBoxList[1].Text))
        {
            MessageBox.Show("Lütfen tüm alanları doldurduğunuzdan emin olun.");
            return;
        }

        // ComboBox'lardaki seçimlerin kontrolü
        if (comboBoxList[0].SelectedValue == null || comboBoxList[1].SelectedValue == null)
        {
            MessageBox.Show("Lütfen tüm seçimleri yapınız.");
            return;
        ...
        // SQL sorgusu
        string query = @"INSERT INTO Muayeneler (HayvanID, SahipID, KlinikID, Tarih, YapilanIslemler, Notlar)
                     VALUES (@HayvanID, @SahipID, @KlinikID, @Tarih, @YapilanIslemler, @Notlar)";
        ...
        try
        {
            ...
            command.Parameters.AddWithValue("@HayvanID", Convert.ToInt32(comboBoxList[1].SelectedValue));
            command.Parameters.AddWithValue("@SahipID", Convert.ToInt32(comboBoxList[0].SelectedValue));
            command.Parameters.AddWithValue("@KlinikID", Convert.ToInt32(comboBoxList[2].SelectedValue));
            command.Parameters.AddWithValue("@Tarih", dateTimePickerList[0].Value.ToString("yyyy-MM-dd HH:mm:ss.fff"));
            command.Parameters.AddWithValue("@YapilanIslemler", textBoxList[0].Text); // Yapılan işlemler
            command.Parameters.AddWithValue("@Notlar", textBoxList[1].Text); // Notlar
